...
/**
 * Title / subtitle chrome metrics — responsive to soft-wrap + font size.
 * Gap between title and subtitle scales with type size (never a flat few px).
 */
export function titleChromeMetrics(theme, data = {}) {
  const raw = String(data.title ?? "").replace(/\n+$/, "");
  const hasSub = Boolean(data.subtitle);
  const titleSize = theme.headingPx ? Math.min(32, theme.headingPx) : 28;
  const subSize = theme.captionPx ?? 15;
  const logoW = theme.logoPath
    ? (theme.logoWidthPx ?? 44) + (theme.logoRightPx ?? 36) + 20
    : 56;
  const titleW = 1280 - 42 - logoW;
  const lineCount = countWrappedLines(raw, titleW, titleSize);
  const lineH = Math.round(titleSize * 1.22);
  const top = 26;
  const titleH = Math.max(lineH + 6, lineH * lineCount);
  // Breath between title block and subtitle — proportional to type
  const titleSubGap = hasSub ? Math.max(28, Math.round(titleSize * 0.72)) : 0;
  const subH = hasSub
    ? Math.max(30, measureLineHeight(String(data.subtitle), titleW - 20, subSize))
    : 0;
  const gapAfter = Math.max(20, theme.space?.lg ?? 24);
...
function countWrappedLines(text, width, fontSize) {
  ...
  return str.split(/\n/).reduce((sum, p) => {
    if (!p) return sum + 1;
    return sum + Math.max(1, Math.ceil(p.length / charsPerLine));
  }, 0) || 1;
}

function measureLineHeight(text, width, fontSize) {
  const str = String(text ?? "");
  if (!str) return Math.round(fontSize * 1.4);
  const lines = countWrappedLines(str, width, fontSize);
...
/**
 * Relative width preference for nested layouts in cols-2 compose.
 * Wider layouts (matrix, RACI) get more horizontal room.
 */
export function layoutWidthWeight(name) {
...
export function partitionRegion(region, arrangement, n, { gap = 16, weights = null } = {}) {
  ...
  if (key === "cols-2" || key === "cols-3") {
    ...
    if (weights?.length >= count) {
      return weightedSplit(region, weights.slice(0, count), "row", g);
    }
...
/** Split along an axis using relative weights (normalized). */
function weightedSplit(region, weights, direction, gap) {
  const n = Math.max(weights.length, 1);
  const total = weights.reduce((a, b) => a + (Number(b) || 1), 0) || n;
  const free = direction === "row"
    ? region.w - gap * (n - 1)
    : region.h - gap * (n - 1);
  let cursor = direction === "row" ? region.x : region.y;
  const cells = [];
  let used = 0;
  for (let i = 0; i < n; i++) {
    const share = Number(weights[i]) || 1;
    const size = i === n - 1
      ? free - used
      : Math.max(40, Math.floor((free * share) / total));
    used += size;
    if (direction === "row") {
      cells.push({ x: cursor, y: region.y, w: size, h: region.h });
      cursor += size + gap;
    } else {
      cells.push({ x: region.x, y: cursor, w: region.w, h: size });
      cursor += size + gap;
    }
  }
  return cells;
