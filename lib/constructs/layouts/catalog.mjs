...
const chrome = (slide, theme, data, page) =>
  title(slide, theme, data.title ?? "", data.subtitle, page ?? 1, data.footerLabel);
...
registerLayout("cover", {
  ...
  paint(slide, theme, boxes, data) {
    label(slide, theme, data.eyebrow ?? "ROADMAP", 42, 48, 320, theme.accent);
    text(slide, theme, data.title ?? "", 42, 120, 760, 220, Math.min(48, theme.titlePx ?? 46), { bold: true });
    if (data.subtitle) text(slide, theme, data.subtitle, 42, 360, 720, 70, 20, { color: theme.muted });
    if (data.chip) chip(slide, theme, data.chip, 42, 460, 130, theme.white, theme.bg);
    if (data.meta) text(slide, theme, data.meta, 42, 512, 700, 36, 15, { color: theme.muted });
    ...
    if (panel !== "none" && panel !== "false") {
      ...
      if (panel === "signals" || panel === "dots") {
        ...
        stats.slice(0, 4).forEach((s, i) => {
          const y = r.y + 70 + i * 120;
          box(slide, theme, r.x + 28, y, r.w - 56, 100, theme.panelAlt, theme.rule, true);
          label(slide, theme, s.label ?? s.h ?? "", r.x + 48, y + 18, r.w - 96, theme.muted);
          text(slide, theme, String(s.value ?? s.v ?? ""), r.x + 48, y + 44, r.w - 96, 40, 22, { bold: true });
        });
        ...
        if (data.media?.path || data.image) {
          imageContain(
            slide,
            theme,
            data.media?.path ?? data.image,
            r.x + 16,
            r.y + 16,
            r.w - 32,
            r.h - 32,
    ...
    if (data.footerLabel || theme.footerText) footer(slide, theme, "", data.footerLabel);
  },
});
...
registerLayout("title-body", {
  ...
  paint(slide, theme, boxes, data, page) {
    chrome(slide, theme, data, page);
    if (data.body) text(slide, theme, data.body, boxes.region.x, boxes.region.y, boxes.region.w, 80, 18, { color: theme.muted });
    paintBullets(slide, theme, itemsOf(data), { ...boxes.region, y: boxes.region.y + (data.body ? 100 : 0), h: boxes.region.h - (data.body ? 100 : 0) });
  },
});
...
for (const [name, reverse] of [["split", false], ["split-reverse", true]]) {
  registerLayout(name, {
    ...
    paint(slide, theme, boxes, data, page) {
      ...
      paintMedia(slide, theme, data.media?.path ?? data.slots?.right?.path ?? data.slots?.left?.path ?? data.slots?.main?.path ?? boxes.mediaPath, boxes.media);
    },
  });
}
...
registerLayout("callout", {
  ...
  paint(slide, theme, boxes, data, page) {
    chrome(slide, theme, data, page);
    box(slide, theme, boxes.region.x, boxes.region.y, 12, boxes.region.h, theme.accent);
    text(slide, theme, data.body ?? valueOf(itemsOf(data)[0]), boxes.region.x + 42, boxes.region.y + 36, boxes.region.w - 80, boxes.region.h - 72, 28, { bold: true, valign: "mid" });
  },
});
...
for (const name of ["figure-focus", "mermaid-focus"]) {
  registerLayout(name, {
    ...
    paint(slide, theme, boxes, data, page) {
      chrome(slide, theme, data, page);
      paintMedia(slide, theme, data.media?.path, boxes.region);
    },
  });
}
...
registerLayout("list-cards", {
  ...
  paint(slide, theme, boxes, data, page) {
    ...
    boxes.cards.forEach((card, i) => {
      ...
      (card.item.bullets ?? []).slice(0, 3).forEach((item, j) => bullet(slide, theme, valueOf(item), card.x + 18, card.y + 88 + j * 30, card.w - 36, theme.ink));
    });
  },
});
...
registerLayout("chart-callout", {
  ...
  paint(slide, theme, boxes, data, page) {
    chrome(slide, theme, data, page);
    paintMedia(slide, theme, data.media?.path, boxes.chart);
