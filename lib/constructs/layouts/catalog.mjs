...
import { autofitFont, measureTextHeight } from "../fit.mjs";
...
const cardText = (slide, theme, item, card, accent, font = 16) => {
  ...
  const padX = 18;
  const labelH = Math.min(28, Math.max(18, Math.floor(card.h * 0.22)));
  label(slide, theme, heading, card.x + padX, card.y + 12, card.w - padX * 2, accent);
  if (detail) {
    const detailY = card.y + 12 + labelH;
    const detailH = Math.max(20, card.h - (detailY - card.y) - 12);
    const fit = autofitFont(detail, card.w - padX * 2, detailH, {
      minFont: 11,
      maxFont: Math.min(font, 16),
      pad: 4,
    });
    text(slide, theme, detail, card.x + padX, detailY, card.w - padX * 2, detailH, fit, {
      color: theme.ink,
      bold: true,
    });
  }
...
const paintBullets = (slide, theme, items, region, color) => {
  ...
  const height = Math.min(48, Math.floor(region.h / rows));
  const font = Math.min(15, Math.max(11, height - 10));
  ...
  paint(slide, theme, boxes, data, page) {
    ...
    const titleLines = Math.max(
      ...
      String(data.title ?? "")
        ...
        .reduce((sum, p) => {
          if (!p) return sum + 1;
          const cpl = Math.max(1, Math.floor(titleW / (titleSize * 0.58)));
          return sum + Math.max(1, Math.ceil(p.length / cpl));
        }, 0),
    );
    const lineH = Math.round(titleSize * 1.18);
    const titleH = titleLines > 1 ? lineH * titleLines + 4 : 52;
    const titleSubGap = Math.max(28, Math.round(titleSize * 0.7));
    text(slide, theme, data.title ?? "", 42, 88, titleW, titleH, titleSize, { bold: true });
    let y = 88 + titleH + titleSubGap;
    if (data.subtitle) {
      const subH = Math.max(
        40,
        measureTextHeight(data.subtitle, 720, 17, { lineHeight: 1.4 }),
      );
      text(slide, theme, data.subtitle, 42, y, 720, subH, 17, { color: theme.muted });
      y += subH + 20;
    }
    if (data.chip) {
...
registerLayout("closing", {
  ...
  paint(slide, theme, boxes, data, page) {
    ...
    const titleH = Math.max(
      56,
      measureTextHeight(data.title ?? "", 1000, titleSize, { lineHeight: 1.2 }),
    );
    text(slide, theme, data.title ?? "", 42, 120, 1000, titleH, titleSize, { bold: true });
    let y = 120 + titleH + 28;
    if (data.subtitle) {
      const subH = Math.max(36, measureTextHeight(data.subtitle, 900, 20));
      text(slide, theme, data.subtitle, 42, y, 900, subH, 20, { color: theme.muted });
      y += subH + 16;
    }
    const photo = mediaPathOf(data);
    if (photo) {
      imageContain(slide, theme, photo, 900, 120, 280, 160);
    }
...
registerLayout("section", {
  place: () => ({ center: { x: 160, y: 200, w: 960, h: 280 } }),
  paint(slide, theme, boxes, data, page) {
    label(slide, theme, data.eyebrow ?? "SECTION", 42, 48, 250, theme.accent);
    const titleSize = 48;
    const titleH = Math.min(
      140,
      Math.max(64, measureTextHeight(data.title ?? "", boxes.center.w, titleSize, { lineHeight: 1.15 })),
    );
    const titleY = boxes.center.y + Math.max(0, Math.floor((boxes.center.h - titleH - (data.subtitle ? 80 : 0)) / 2));
    text(slide, theme, data.title ?? "", boxes.center.x, titleY, boxes.center.w, titleH, titleSize, {
      bold: true,
      align: "center",
      valign: "mid",
    });
    if (data.subtitle) {
      const subY = titleY + titleH + 32;
      text(slide, theme, data.subtitle, boxes.center.x, subY, boxes.center.w, 48, 18, {
        color: theme.muted,
        align: "center",
      });
    }
...
registerLayout("big-number", {
  place: (ctx) => {
    const region = ctx.fit.contentBox();
    const items = itemsOf(ctx.data);
    const n = Math.max(items.length, 1);
    const nested = ctx.fit.nested || ctx.data._suppressChrome;
    const gap = nested ? 12 : Math.max(ctx.space.cardGap ?? 20, 24);
    const align = resolveVAlign(ctx.data, nested);
    // Stack when nested cards would be too narrow for KPI values
    const minCardW = 175;
    const stack =
      nested &&
      n >= 2 &&
      region.w < n * minCardW + gap * (n - 1);

    if (stack) {
      const cardH = Math.min(140, Math.max(108, Math.floor((region.h - gap * (n - 1)) / n)));
      const blockH = n * cardH + gap * (n - 1);
      let y0 = region.y;
      if (align === "center") y0 += Math.max(0, Math.floor((region.h - blockH) / 2));
      else if (align === "end") y0 += Math.max(0, region.h - blockH);
      ...
      return { region, cards, stacked: true };
    }

    const cols = Math.min(n, 4);
    const cardH = nested
      ? Math.min(180, Math.max(130, Math.floor(region.h * 0.68)))
      : Math.min(300, Math.max(220, region.h - 24));
    let y = region.y;
    if (align === "center") y += Math.max(0, Math.floor((region.h - cardH) / 2));
    else if (align === "end") y += Math.max(0, region.h - cardH);
    const cards = ctx.fit
      .distributeRow({ ...region, h: cardH, y }, cols, gap)
      .slice(0, n)
      .map((c, i) => ({
        ...c,
        item: items[i],
    ...
    return { region, cards, stacked: false };
  },
  paint(slide, theme, boxes, data, page) {
    chrome(slide, theme, data, page);
    boxes.cards.forEach((card, i) => {
      ...
      const labelH = 22;
      const detail = detailOf(item);
      const detailH = detail ? 32 : 0;
      const gap = 8;
      const valueY = card.y + barH + padY + labelH + gap;
      const valueH = Math.max(
        36,
        card.h - barH - padY - labelH - gap - (detail ? detailH + 6 : padY) - padY,
      );
      const valueStr = String(item.value ?? item.v ?? "");
      const valueFont = autofitFont(valueStr, card.w - padX * 2, valueH, {
        minFont: 14,
        maxFont: card.w < 220 ? 26 : 40,
        pad: 2,
      });
      const needsWrap =
        measureTextHeight(valueStr, card.w - padX * 2, valueFont) > valueH * 0.85;
...
/** Vertical align for nested / compose panels: start | center | end */
function resolveVAlign(data, nested = false) {
...
registerLayout("callout", {
  ...
  paint(slide, theme, boxes, data, page) {
    chrome(slide, theme, data, page);
    const textBox = boxes.text ?? boxes.region;
    const body = data.body ?? valueOf(itemsOf(data)[0]);
    const font = autofitFont(body, textBox.w - 80, textBox.h - 48, {
      minFont: 14,
      maxFont: textBox.h < 200 ? 20 : 28,
      pad: 8,
    });
    box(slide, theme, textBox.x, textBox.y, 12, textBox.h, theme.accent);
    text(slide, theme, body, textBox.x + 36, textBox.y + 20, textBox.w - 64, textBox.h - 40, font, {
      bold: true,
      valign: "mid",
    });
    if (boxes.media) paintMedia(slide, theme, mediaPathOf(data), boxes.media);
...
registerLayout("steps-h", {
  ...
  place: (ctx) => {
    const region = ctx.fit.contentBox();
    const n = Math.min(itemsOf(ctx.data).length || 1, 5);
    const nested = ctx.fit.nested || ctx.data._suppressChrome;
    // In narrow compose cells, stack vertically instead of crushing cards
    if (nested && n > 2 && region.w / n < 155) {
      const cardH = Math.min(88, Math.max(64, Math.floor((region.h - 10 * (n - 1)) / n)));
      const rows = ctx.fit.distributeCol(
        { ...region, h: Math.min(region.h, n * cardH + 10 * (n - 1)) },
        n,
        10,
      );
      return { cards: rows, stacked: true };
    }
    const cardH = nested ? Math.min(110, Math.max(80, Math.floor(region.h * 0.5))) : region.h;
    const y = nested ? region.y : region.y;
    return {
      cards: ctx.fit.distributeRow({ ...region, y, h: cardH }, n, nested ? 12 : 18),
      stacked: false,
    };
  },
  paint(slide, theme, boxes, data, page) {
    ...
    boxes.cards.forEach((card, i) => {
      if (i && !boxes.stacked) {
        line(slide, theme, boxes.cards[i - 1].x + boxes.cards[i - 1].w, card.y + card.h / 2, card.x, card.y + card.h / 2, theme.rule, 2);
      }
...
registerLayout("list-cards", {
  ...
  place: (ctx) => {
    const nested = ctx.fit.nested || ctx.data._suppressChrome;
    const region = ctx.fit.contentBox();
    const opts = nested && region.w < 640
      ? { maxCols: 1, preferCols: 1 }
      : { maxCols: 4 };
    return gridPlace(ctx, opts);
  },
  paint(slide, theme, boxes, data, page) {
    chrome(slide, theme, data, page);
    boxes.cards.forEach((card, i) => {
      ...
      if (bullets.length && bulletY < card.y + card.h - 24) {
        const avail = card.y + card.h - 12 - bulletY;
        const rowH = Math.min(28, Math.floor(avail / bullets.length));
        bullets.forEach((b, j) =>
          bullet(slide, theme, valueOf(b), card.x + 18, bulletY + j * rowH, card.w - 36, theme.ink, {
            h: rowH - 2,
            font: Math.min(13, rowH - 8),
          }),
        );
      }
    });
...
registerLayout("banded-list", {
  ...
  place: (ctx) => {
    const region = ctx.fit.contentBox();
    const n = Math.min(itemsOf(ctx.data).length || 1, 6);
    const nested = ctx.fit.nested || ctx.data._suppressChrome;
    if (!nested) {
      return { rows: ctx.fit.distributeCol(region, n, 10) };
    }
    const rowH = Math.min(56, Math.max(40, Math.floor((region.h - 10 * (n - 1)) / n)));
    const y0 = region.y;
  ...
  paint(slide, theme, boxes, data, page) {
    ...
    boxes.rows.forEach((row, i) => {
      ...
      text(slide, theme, valueOf(item), row.x + 28, row.y + Math.max(6, Math.floor((row.h - 22) / 2)), row.w * 0.38, 22, Math.min(16, row.h - 14), { bold: true });
      text(slide, theme, item?.value ?? detailOf(item), row.x + row.w * 0.4, row.y + Math.max(6, Math.floor((row.h - 22) / 2)), row.w * 0.55, 22, Math.min(14, row.h - 16), { color: theme.muted, align: "right" });
    });
  },
});
...
registerLayout("matrix2x2", {
  ...
  place: (ctx) => {
    const region = ctx.fit.contentBox();
    const nested = ctx.fit.nested || ctx.data._suppressChrome;
    const leftPad = nested
      ? (region.w < 640 ? 52 : 72)
      : 120;
    const bottomPad = nested
      ? (region.h < 300 ? 28 : 40)
      : 70;
    const topPad = nested ? 4 : 12;
    const rightPad = nested ? 12 : 30;
    return {
      region,
      grid: {
        x: region.x + leftPad,
        y: region.y + topPad,
        w: Math.max(120, region.w - leftPad - rightPad),
        h: Math.max(80, region.h - topPad - bottomPad),
      },
      axisFont: nested && region.w < 640 ? 11 : 14,
    };
  },
  paint(slide, theme, boxes, data, page) {
    chrome(slide, theme, data, page);
    const { grid } = boxes;
    const axisFont = boxes.axisFont ?? 14;
    ...
    text(slide, theme, data.axes?.y ?? data.yAxis ?? "HIGH", boxes.region.x, grid.y, Math.max(48, grid.x - boxes.region.x - 6), 28, axisFont, {
      color: theme.muted,
      bold: true,
    });
    text(slide, theme, data.axes?.x ?? data.xAxis ?? "LOW → HIGH", grid.x, grid.y + grid.h + 6, grid.w, 24, axisFont, {
      color: theme.muted,
      bold: true,
      align: "center",
    });
    ...
    Object.entries(byQuad).forEach(([key, items]) => {
      ...
      items.forEach((item, i) => {
        const y = top + i * rowH;
        const h = Math.max(28, rowH - 6);
        const accent = colorOf(theme, i);
        const itemLabel = valueOf(item);
        const font = autofitFont(itemLabel, cell.w - 56, h, { minFont: 11, maxFont: 15, pad: 4 });
        box(slide, theme, cell.x + 12, y, cell.w - 24, h, theme.panelAlt, accent, true);
        text(slide, theme, itemLabel, cell.x + 22, y, cell.w - 44, h, font, {
          bold: true,
          color: theme.ink,
          valign: "mid",
        });
