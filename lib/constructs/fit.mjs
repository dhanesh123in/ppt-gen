 ...
 */
export function measureTextHeight(text, width, fontSize, { lineHeight = 1.4, charFactor = 0.58 } = {}) {
  const str = String(text ?? "");
  if (!str || width <= 0) return 0;
  const avgChar = fontSize * charFactor;
  const charsPerLine = Math.max(1, Math.floor((width * 0.94) / avgChar));
...
export function fitGrid(region, items, opts = {}) {
  ...
  let cellW = Math.floor((region.w - gap * (cols - 1)) / cols);
  let cellH = Math.floor((region.h - gap * (rows - 1)) / rows);
  if (opts.maxCellH != null && cellH > opts.maxCellH) {
    cellH = opts.maxCellH;
  }
  const blockH = rows * cellH + gap * (rows - 1);
  const blockW = cols * cellW + gap * (cols - 1);
  let originY = region.y;
  let originX = region.x;
  if (opts.alignY === "center") {
    originY = region.y + Math.max(0, Math.floor((region.h - blockH) / 2));
  } else if (opts.alignY === "end") {
    originY = region.y + Math.max(0, region.h - blockH);
  }
  if (opts.alignX === "center") {
    originX = region.x + Math.max(0, Math.floor((region.w - blockW) / 2));
  }
...
export function createFitContext(theme, data = {}, opts = {}) {
  ...
  const nested = Boolean(override) || Boolean(data._suppressChrome);
  return {
    theme,
    data,
    space,
    ...
    fitGrid: (region, items, gridOpts) =>
      fitGrid(region, items, {
        minGap: space.cardGap,
        minFont: space.minFont,
        maxFont: nested ? Math.min(16, space.maxFont) : space.maxFont,
        maxCols: nested && region.w < 640 ? 2 : space.maxCols,
        ...(nested
          ? {
              maxCellH: Math.min(170, Math.max(120, Math.floor(region.h * 0.42))),
              alignY: "start",
              preferCols: region.w < 520 ? 1 : undefined,
            }
          : {}),
        ...gridOpts,
      }),
