 ...
 * @param {{ contentRegion?: object, suppressChrome?: boolean }} [opts]
 */
export function runLayout(slide, theme, name, data, page = 1, opts = {}) {
  ...
  if (opts.suppressChrome) {
    normalized = { ...normalized, _suppressChrome: true };
  }
  const fit = createFitContext(theme, normalized, {
    contentRegion: opts.contentRegion ?? null,
  });
  const ctx = {
    theme,
    data: normalized,
...
/** Nested layout inside a compose region (no slide chrome). */
export function runNestedLayout(slide, theme, name, data, page, region) {
  return runLayout(slide, theme, name, data, page, {
