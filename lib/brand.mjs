...
import { resolveAssetPath } from "./constructs/assets.mjs";
...
/**
 * Resolve branding.logo / logo_path from tokens (or generate default SVG).
 * @returns {Promise<{path: string|null, generated: boolean}>}
 */
export async function resolveLogoSource(tokens, { logoOverride = null } = {}) {
  const branding = tokens.raw?.branding ?? {};
  const custom = logoOverride ?? branding.logo ?? branding.logo_path ?? null;
  if (custom === false || custom === "none" || custom === "false") {
    return { path: null, generated: false };
  }
  if (custom) {
    const resolved = resolveAssetPath(custom, { themeName: tokens.name });
    if (!resolved) {
      throw new Error(
        `Custom logo not found: ${custom} (tried theme dir, assets/brand/, repo root)`,
      );
    }
...
/**
 * Write logo into theme output dir. Uses custom file when branding.logo is set;
 * otherwise generates the procedural SVG mark.
 * @returns {Promise<string|null>} path to written logo (or null if disabled)
 */
export async function ensureLogo(outPath, tokens, { logoOverride = null } = {}) {
  const source = await resolveLogoSource(tokens, { logoOverride });
  await mkdir(dirname(outPath), { recursive: true });

  if (!source.path && !source.generated) {
    return null;
  }
  ...
  if (source.path) {
    ...
    const dest =
      extname(outPath).toLowerCase() === ext
        ? outPath
        : join(dirname(outPath), `logo${ext}`);
    await copyFile(source.path, dest);
    return dest;
  }

  await writeFile(outPath, buildLogoSvg(tokens));
  return outPath;
}
