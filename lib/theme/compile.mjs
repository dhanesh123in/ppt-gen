...
async function logoDataUriFromFile(path) {
  if (!path || !existsSync(path)) return null;
  const ext = extname(path).toLowerCase();
  const buf = await readFile(path);
  if (ext === ".svg") return logoDataUri(buf.toString("utf8"));
  const mime =
    ext === ".jpg" || ext === ".jpeg"
      ? "image/jpeg"
      : ext === ".webp"
...
export async function compileTheme(name) {
  ...
  const logoHint = join(outDir, "logo.svg");
  const logoPath = await ensureLogo(logoHint, tokens);
  const brandDir = join(ASSETS_DIR, "brand");
  await mkdir(brandDir, { recursive: true });

  let logoUri = null;
  if (logoPath) {
    const brandCopy = join(brandDir, basename(logoPath));
    await writeFile(brandCopy, await readFile(logoPath));
    logoUri = await logoDataUriFromFile(logoPath);
  }
