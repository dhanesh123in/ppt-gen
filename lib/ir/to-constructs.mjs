...
async function resolveMediaSlots(layoutData, tokens, data) {
  ...
  if (out.media) {
    const path = await expandRef(out.media);
    if (path) out.media = { ...(typeof out.media === "object" ? out.media : {}), path };
  }
  if (out.slots) {
    out.slots = { ...out.slots };
    for (const key of ["left", "right", "main", "aside"]) {
      if (out.slots[key] != null && typeof out.slots[key] !== "string") {
        ...
        const path = await expandRef(out.slots[key]);
        if (path) out.slots[`${key}Path`] = path;
        // keep original string for bullet text if not a media ref
        if (path) out.slots[key] = { path };
      }
    }
  }
  return out;
