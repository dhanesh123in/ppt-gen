...
async function resolveMediaSlots(layoutData, tokens, data) {
  ...
  if (Array.isArray(out.slots)) {
    out.slots = await Promise.all(
      out.slots.map(async (slot) => {
        ...
        for (const key of ["media", "image", "picture", "photo"]) {
          ...
          if (!path) continue;
          if (key === "media") {
            next.media = { ...(typeof next.media === "object" ? next.media : {}), path };
          } else {
            next[key] = path;
            if (!next.media?.path) next.media = { ...(next.media ?? {}), path };
          }
        }
          ...
          next.items = await Promise.all(
            next.items.map(async (item) => {
              ...
              for (const key of ["image", "picture", "photo"]) {
                ...
                const path = await expandRef(it[key]);
                if (path) it[key] = path;
              }
              return it;
            }),
