...
/**
 * Shared deck IR: frontmatter + slides with markdown segments and ::: construct blocks.
 *
 * Construct fence:
...
export function parseSlideBody(raw) {
  const blocks = [];
  let last = 0;
  const re = new RegExp(CONSTRUCT_RE.source, CONSTRUCT_RE.flags);
  let match;
  while ((match = re.exec(raw)) !== null) {
    ...
    if (yamlText) {
      try {
        ...
        throw new Error(
          `Invalid YAML in ::: construct ${match[1]}: ${err.message}`,
        );
      }
    }
    if (typeof data !== "object" || Array.isArray(data)) {
      throw new Error(`Construct ${match[1]} data must be a YAML mapping`);
    }
    blocks.push({ type: "construct", name: match[1], data });
...
export function parseDeckMarkdown(text) {
  ...
  const hasConstructs = slides.some((s) =>
    s.blocks.some((b) => b.type === "construct"),
  );
  return {
    meta,
    slides,
...
export function resolveEngine(ir, override = null) {
  ...
  return ir.hasConstructs ? "constructs" : "marp";
}
